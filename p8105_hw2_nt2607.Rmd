...
Note: The variable prez_gop should only accept 0 or 1 but i can see a few 2's as well which make cause errors.
...
In the `pols_month_df` dataset we have `r nrow(pols_month_df)` rows and `r ncol(pols_month_df)` columns.

In the `snp_df` dataset we have `r nrow(snp_df)` rows and `r ncol(snp_df)` columns.
